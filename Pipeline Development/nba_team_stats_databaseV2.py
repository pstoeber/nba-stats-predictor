...
def create_insert_statements(connection, season_total_list, table_names, iterated_header_list):
    ...
    for p, i in enumerate(season_total_list):
        ...
        team_info_state = 'insert into ' + table_names[0] + ' values ("' + str(p +1) + '", "' + row[0] + '")'
        sql_execute(connection, [team_info_state])

        points_state = 'insert into ' + table_names[1] + ' values (' + str(p +1) + ', ' + row[1] + ', ' + row[2] + ', ' + row[3] + ')'
        sql_execute(connection, [points_state])

        fg_state = 'insert into ' + table_names[2] + ' values (' + str(p +1) + ', ' + row[4] + ', ' + row[5] + ')'
        sql_execute(connection, [fg_state])

        fg3_state = 'insert into ' + table_names[3] + ' values (' + str(p +1) + ', ' + row[6] + ', ' + row[7] + ', ' + row[8] + ')'
        sql_execute(connection, [fg3_state])

        rb_state = 'insert into ' + table_names[4] + ' values (' + str(p +1) + ', ' + row[9] + ', ' + row[10] + ', ' + row[11] + ')'
        sql_execute(connection, [rb_state])

        to_state = 'insert into ' + table_names[5] + ' values (' + str(p +1) + ', ' + row[12] + ', ' + row[13] + ')'
