...
def espn_id_scrap():
    ...
    with open('nba_links_log.txt', 'r') as infile:
        try:
            ...
            iteration = int(content[-1].split('\t')[0])
        except IndexError:
            iteration = 0
    print(iteration)

    with open('nba_links_log.txt', 'a') as outfile:
        for i in range(iteration,5000000):
            print(i)
            temp_link = id_link + str(i+1)

            try:
                link = requests.get(temp_link, timeout=10)
                ...
                if soup.find('tr', {'class':'total'}) != None:
                    outfile.write(str(i) + '\t' + 'http://www.espn.com/nba/player/stats/_/id/' + str(i) + '\n')
                    link_list.append('http://www.espn.com/nba/player/stats/_/id/' + str(i+1))
            except:
                print('Link timed out: ' + temp_link)
