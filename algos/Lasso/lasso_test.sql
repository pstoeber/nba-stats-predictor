...
inner join team_advanced_boxscore_stats as a_stats on bm.game_hash = a_stats.game_hash and
                                                      bm.target = a_stats.team
left outer join RegularSeasonAverages as reg_avg on basic.player_id = reg_avg.player_id and
                                                    bm.season -1 = reg_avg.season
inner join points as opp_team_pts on bm.opp_id = opp_team_pts.team_id and
                                     bm.season -1 = opp_team_pts.season
inner join team_misc_boxscore_stats as misc on bm.game_hash = misc.game_hash and
                                               bm.target = misc.team
